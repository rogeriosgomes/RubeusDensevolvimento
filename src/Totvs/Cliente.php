...
class Cliente
{
   ...
   function GetCliente($codcfo,
                       ...
                       $dataAtual)
    {
        ...
        $cod_cliente = $req_dump["SaveRecordResult"];//$Resposta_pessoa["SaveRecordResult"]; 
        
        return substr($cod_cliente,2);
    }

    // valida se o responsável financeiro do aluno já existe na tabela de cliente/fornecedor do totvs
    function ValidaCliente($cpf){
  ...
  function CriarCliente($tipoCliente,
                        $nome,
                        $cgccfo,
                        $rua,
                        $numero,
                        ...
                        $dataAtual)
  {
    ...
    if ($tipoCliente == "Próprio Aluno" or $tipoCliente == "Pr\u00f3prio Aluno") 
    {
        $codcfo_repons_finan = $this->ValidaCliente($cgccfo); //verificar depois
        
        $estadocivil ='S';
        
        $codcfo = $this->GetCliente($codcfo_repons_finan,
                                       $nome,
                                       $cgccfo,
                                       $rua,
                                       $numero,
    ...
    if($tipoCliente  ==  "Pai" or $tipoCliente ==  "Mãe" or $tipoCliente  ==  "M\u00e3e")
    {
        ...
        $codPessoa = Pessoa::ValidaSePessoaExiste($nome,
                                                  $dtnascimento,
                                                  $estadoNatal,
                                                  $cidadeNatal,
                                                  $cgccfo);
        ...
        $codPessoa= Pessoa::IncluiPessoa( $codPessoa,
                                          $nome,
                                          $dtnascimento,
                                          $cgccfo,
                                          $estadoNatal,
    ...
    if ($tipoCliente  == "Outro") 
    {
       ...
       $codcfo_repons_finan  = $this->ValidaCliente($ContatatoAluno['cpf_resp_financ'] ); //verificar depois
